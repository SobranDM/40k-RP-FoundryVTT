...
export async function prepareCommonRoll(rollData) {
  const html = await renderTemplate("systems/whfortyrp/template/dialog/common-roll.html", rollData);
 ...
 * @param {object} rollData
 * @param {WHFortyRPActor} actorRef
 */
export async function prepareCombatRoll(rollData, actorRef) {
    const html = await renderTemplate("systems/whfortyrp/template/dialog/combat-roll.html", rollData);
    let dialog = new Dialog({
        title: rollData.name,
        content: html,
        buttons: {
...
export async function preparePsychicPowerRoll(rollData) {
  const html = await renderTemplate("systems/whfortyrp/template/dialog/psychic-power-roll.html", rollData);
  let dialog = new Dialog({
    title: rollData.name,
    content: html,
    buttons: {
